    ...
    <div class="space-y-6">
        ...
        <div class="glass-card rounded-xl p-8">
            <div class="text-center mb-8">
                <div
                    class="mx-auto h-20 w-20 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-full flex items-center justify-center mb-4">
                ...
                <p class="text-gray-600 max-w-2xl mx-auto">Nuestro test utiliza inteligencia artificial para analizar tus
                    respuestas y brindarte recomendaciones precisas sobre tu futuro profesional.</p>
            </div>
            ...
            <div class="text-center">
                {{-- Actualizado para redirigir a la página de tests en lugar de mostrar alerta --}}
                <a href="{{ route('tests.index') }}" class="btn-primary text-lg px-8 py-4 inline-block">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="module-card">
                <div
                    class="h-12 w-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center mb-4">
                ...
                <div class="flex items-center justify-between">
                    ...
                    {{-- Actualizado para redirigir a tests --}}
                    <a href="{{ route('tests.index') }}" class="btn-secondary text-sm px-4 py-2">Realizar</a>
                </div>
            </div>
            ...
            <div class="module-card">
                <div
                    class="h-12 w-12 bg-gradient-to-br from-green-500 to-emerald-600 rounded-lg flex items-center justify-center mb-4">
                    <svg class="h-6 w-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="module-card">
                <div
                    class="h-12 w-12 bg-gradient-to-br from-purple-500 to-pink-600 rounded-lg flex items-center justify-center mb-4">
                    <svg class="h-6 w-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="glass-card rounded-xl p-6">
            ...
            {{-- Actualizado para mostrar tests reales del usuario --}}
            @if (auth()->user()->testResults->count() > 0)
                <div class="space-y-4">
                    @foreach (auth()->user()->testResults->take(5) as $result)
                        <div class="flex items-center justify-between p-4 bg-white rounded-lg border border-gray-200">
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    <p class="font-medium text-gray-900">{{ $result->test->title ?? 'Test no disponible' }}
                                    </p>
                                    <p class="text-sm text-gray-500">Completado el
                                        {{ $result->created_at->format('d/m/Y') }}</p>
                                </div>
                            </div>
                            <div class="flex items-center space-x-2">
                                <span
                                    class="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded-full">Completado</span>
                                <a href="{{ route('tests.result', $result->id) }}"
                                    class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">Ver Resultados</a>
                            </div>
                        </div>
                    @endforeach
                </div>
            @else
                <div class="text-center py-8">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900">No hay tests completados</h3>
                    <p class="mt-1 text-sm text-gray-500">Comienza tu primer test vocacional para descubrir tu carrera
                        ideal.</p>
                    <div class="mt-6">
                        <a href="{{ route('tests.index') }}" class="btn-primary">
                            Realizar Test
                        </a>
                    </div>
                </div>
